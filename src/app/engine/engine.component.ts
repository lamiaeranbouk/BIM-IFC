...
export class EngineComponent implements OnInit, OnDestroy {
  ...
  public linkInserted(url) {
    console.log('link inserted function called with URL ', url);
  ...
  // Read as text for ifcText and metadata
  const textReader = new FileReader();
  textReader.onload = (event) => {
    if (event.target && event.target.result) {
      this.ifcText = event.target.result as string;
      this.fetchMetadataFromIFC(this.ifcText);
    } else {
      this.isErrorHappened = true;
      this.isUploading = false;
      this.isLoading = false;
      this.loadingMessage = 'Failed to read IFC file text';
    }
  };
  textReader.onerror = () => {
    this.isErrorHappened = true;
    ...
    console.error('FileReader text error');
  };
  ...
  try {
    await this.ifc.setWasmPath('./assets/'); // Ensure WASM files are in ./assets/
    await this.ifcLoader.load(url, async (ifcModel: IFCModel) => {
      ...
      const data = await this.spatialStructure;
      if (data.children.length > 0) {
        ...
        await this.findAllChildrenTypes(data.children, data.expressID);
      } else {
        console.error('Empty IFC selected');
        this.isErrorHappened = true;
        this.loadingMessage = 'Empty or invalid IFC file';
      }

      for (const type of this.types) {
        if (!this.filteredTypes.find(o => o.type === type.type && type.type)) {
          this.filteredTypes.push(type);
        }
      }
      this.expressIDOfPreviousLevel = 0;
      await this.populateIFCCategories();
      await this.setupAllCategories();
      this.isLoading = false;
      this.loadingMessage = '';
    }, (progress) => {
      this.isLoading = true;
    ...
    this.isErrorHappened = true;
    this.isLoading = false;
    this.loadingMessage = 'Failed to load IFC model. Please try another file.';
  }
}
